   ...
   const RentalModal = ({ isOpen, onClose, selectedCar, refreshBookings }) => {
    ...
    useEffect(() => {
      if (formData.pickupDate && formData.returnDate && selectedCar) {
        ...
        // Allow same-day booking (diffDays === 0) and charge 1 day price
        if (diffDays >= 0) {
        ...
        <button
          ...
          className={`group relative flex flex-col items-center gap-3 p-4 rounded-2xl border-2 transition-all duration-300 min-w-[100px] ${
            isUnavailable
              ...
              : 'hover:bg-gray-50 hover:shadow-md cursor-pointer border-gray-200 hover:border-gray-300'
          } ${isSelected ? 'ring-3 ring-black ring-opacity-20 bg-gray-50 shadow-lg border-gray-400' : ''}`}
        >
          <div
          ...
          {isUnavailable && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="bg-red-500 text-gray-50 text-xs px-2 py-1 rounded-full font-medium">
                Unavailable
              </div>
            </div>
          )}
      ...
      <div className="relative h-60 w-full bg-gray-100">
        ...
        {!isActuallyAvailable && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/40">
            <span className="text-white font-bold text-lg px-4 py-2 rounded-lg bg-black/60">
              Not Available
            </span>
          </div>
        )}
