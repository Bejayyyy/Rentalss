            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl text-lg font-semibold shadow-lg transition-colors duration-200 ${
                isRentButtonDisabled
                  ...
                  : "bg-black text-gray-50 hover:bg-gray-800 hover:shadow-xl"
              }`}
            >
...
const RentalModal = ({ isOpen, onClose, selectedCar }) => {
  const [formData, setFormData] = useState({
    fullName: "", email: "", phone: "", pickupDate: "", returnDate: "",
    pickupLocation: "", licenseNumber: "", vehicleVariantId: ""
  })
  const [variants, setVariants] = useState([])
  const [selectedVariant, setSelectedVariant] = useState(null)
  ...
  useEffect(() => {
    if (formData.pickupDate && formData.returnDate && selectedCar) {
      ...
      if (diffDays >= 0) {
        ...
        setRentalDays(days)
        setTotalPrice(days * (selectedCar.price_per_day || 0))
      } else {
        setRentalDays(0)
        setTotalPrice(0)
      }
            ...
            <div className="lg:w-2/5 bg-gray-50 pl-5 pr-5 flex flex-col">
              <div className="flex items-center justify-center mt-22.5">
                <div className="w-full h-80 lg:h-96 rounded-3xl shadow-lg flex items-center justify-center overflow-hidden border border-gray-100 relative">
                  <img src={defaultBackground} className="absolute inset-0 w-full h-full object-cover" />
              ...
              <div className="text-center mt-5 ml-1 lg:text-left mb-2">
                ...
                <span className="font-semibold text-gray-900">{selectedVariant?.color || "Unavailable"}</span>
              </div>
              ...
              <div className="flex gap-3 justify-center ml-1 lg:justify-start">
                {variants.map((variant) => {
                  ...
                  else if (colorName.includes("gold") || colorName.includes("brown")) bgColor = "#d1a054"
                  ...
                  return (
                    <div key={variant.id} className="relative">
                      <button
                        ...
                        className={`w-5 h-5 rounded-full ${isSelected ? "ring-1 ring-gray-500" : ""} ${
              ...
              <div className="mt-4 text-center lg:text-left ml-1 text-black">
                <h3 className="text-3xl font-bold mb-3">{displayCar?.make} {displayCar?.model}</h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-2xl p-8 text-center hover:border-gray-500 hover:bg-gray-50 transition-all duration-300 bg-white/50">
                      <input
                        ...
                        onChange={handleFileChange} className="hidden" id="govId"
                      />
                      <label htmlFor="govId" className="cursor-pointer block">
                        {govIdPreview ? (
                          <img
                            src={govIdPreview} alt="ID Preview"
                ...
                <div className="flex justify-between items-center bg-gray-100 p-6 rounded-2xl border border-gray-200">
                  <div>
                    ...
                    <div className="text-4xl font-extrabold text-black mt-1">₱{totalPrice.toLocaleString()}</div>
                  </div>
                  <button
                    type="submit"
...
const CarCard = ({ car, onRentClick, onOpenDetails }) => {
  const [variantStats, setVariantStats] = useState({
    totalAvailable: 0, totalQuantity: 0, hasVariants: false,
    ...
    const fetchVariantAvailability = async () => {
      ...
      try {
        ...
        if (error) {
          ...
          const totalAvailable = variants.reduce((sum, variant) => sum + (variant.available_quantity || 0), 0)
          const totalQuantity = variants.reduce((sum, variant) => sum + (variant.total_quantity || 0), 0)
          setVariantStats({
            totalAvailable, totalQuantity, hasVariants: true,
  ...
  return (
    ...
    >
      <div className="relative h-60 w-full bg-gray-100">
        <img src={defaultBackground} alt="Default background" className="absolute inset-0 w-full h-full object-cover rounded-t-2xl" />
        <img
          ...
          className={`relative w-full h-full object-contain p-2 transition-transform duration-500 group-hover:scale-105 ${
            !isActuallyAvailable ? "opacity-70" : ""
          }`}
        />
        {!isActuallyAvailable && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/40">
            <span className="text-white font-bold text-lg px-4 py-2 rounded-lg bg-black/60">Not Available</span>
          </div>
        )}
      </div>
            ...
            {isLoading ? (
              ...
              <>
                ...
                {variantStats.variantCount > 2 && (
                  <div className="text-xs text-gray-400">+{variantStats.variantCount - 2} more colors</div>
                )}
              </>
            ) : (
